...
function Page() {
    ...
    const handleUpdateStatus = async () => {
        if (selectedEnrollmentId) {
            // Gọi API hoặc cập nhật status ở đây
            const response = await axios.patch(`${Endpoints.baseApiURL.URL}/${Endpoints.Enrollment.UPDATE_STATUS_ENROLL(selectedEnrollmentId)}`)
                        ...
                        {students &&
                            ...
                            students.map((student, index) => (
                                <TableRow key={student.studentID}>
                                    ...
                                    <TableCell >
                                        <Button onClick={() => handleOpenDialog(student.enrollmentId)} variant={student.status == 1 ? 'outline' : 'secondary'}>{student.status == 1 ? 'đã đăng kí' : 'chưa đăng kí'}</Button>
                                    </TableCell>
                                </TableRow>
                            ))}
                    </TableBody>
